...
import requests, re, json, pymysql,hashlib
...
while True:
    ...
    for i in a:
        ...
        jpg_re = requests.get(jpg_url,verfiy=False)
        ...
        with open("/home/ceshi_jpg/{}.jpg".format(jgp_name),"wb") as f:
            f.write(jpg)
        image_path = "/home/ceshi_jpg/{}.jpg".format(jgp_name)
        ss.input("'/home/ceshi_video/{}.mp4'.format(video_name)")
        size = ss.video_info()["format"]['size']

        with db.cursor() as cursor:
            try:
                sql = "INSERT INTO 'video_copy1'('source','ref_id','video_path','image_path','title','size','status','video_id_test','video_id_prod') values('美拍美妆',%s,%s,%s,%s,%s,%s,0,0,0)",(id,video_path,image_path,caption,caption,size)
                cursor.execute(sql)
        ...
        try:
            for i in range(1,7):
                ...
                for i in c:
                    ...
                    try:
                        sql = "INSERT INTO 'comment_copy1'('source','ref_id','content','user_id_test','video_id_test','user_id_prod','video_id_prod','status') values('美拍美妆',%s,%s,0,0,0,0,0)", (id,comments)
                        cursor.execute(sql)
                    except Exception as e:
                        print(sql)
                        print("eeeeeee", e)
